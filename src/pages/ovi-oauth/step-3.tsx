...
export default function OAuthStep3() {
  const { accessToken, expiryTime, refreshToken, error, description, errorUri } = useLoaderData() as any
  ...
  document.title = t('OAuth Step 3 - {{title}}', { title: siteConfig.title })
          ...
          <div>
            ...
            <div className="mb-2 mt-6 text-right">
              <button
                ...
                onClick={() => {
                  navigate('/onedrive-vercel-index-oauth/step-1')
                }}
              >
...
export async function loader({ params }) {
  const authCode = params.code //useLocation().state.authCode
  /*const authCode = window.location.search
    .slice(1)
    .split('&')
    .find(pair => pair.startsWith('authCode='))
    ?.split('=')[1]*/
